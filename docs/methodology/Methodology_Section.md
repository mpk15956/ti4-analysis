...
**RQ2.** $H_0$: NSGA-II's hypervolume does not exceed that of scalar algorithms (RS, HC, SA, SGA, TS) under the canonical formulation at equal total evaluation budget (Wilcoxon signed-rank, one-tailed, Holm–Bonferroni corrected). **Result:** Tested under the canonical multi-algorithm run (`submit_paper1_multialgo.sh`); results integrated into this section once that run completes.
