...
- Pooled across all five conditions × eight budgets ($n = 12{,}000$ rows), the spatial trade-offs are recovered: $\rho(\text{balance\_gap}, \text{Moran's I}) = -0.415$, $\rho(\text{balance\_gap}, \text{LSAP}) = -0.436$, $\rho(\text{balance\_gap}, \text{JFI}) = -0.676$ (all $p < 10^{-300}$). Maps with lower balance gap also have lower spatial autocorrelation across the cross-condition cohort, consistent with the §3.9 finding that the full composite improves all three dimensions jointly.

**RQ4.** $H_0$: at the canonical hyperparameters, **evaluations-to-best** does not differ across algorithms under the canonical formulation (Friedman test, paired by seed). We use `evals_to_best` (the iteration count at which the run-best composite was first observed; recorded in `results.csv`) rather than wall-clock seconds because evaluation count factors out implementation overhead and cluster-architecture heterogeneity, which are the canonical anytime-performance metric in the metaheuristics literature (Hutter, Kotthoff, & Vanschoren, 2019). Wall-clock (`elapsed_sec`) is also recorded and reported as a secondary descriptive statistic per algorithm, but the formal Friedman test runs on `evals_to_best`. **Result:** Rejected. At the canonical RQ4 budget $b = 200{,}000$ (the largest budget at which all six algorithms carry a real, non-sentinel `evals_to_best`), the six-way Friedman omnibus is $\chi^2 = 456.80$ ($df = 5$, $n = 100$, $p = 1.7 \times 10^{-96}$), and it is significant at every budget tested from $b = 1{,}000$ to $b = 200{,}000$. NSGA-II reaches its best 1:1:1 composite only late in each run: its median `evals_to_best` at $b = 200{,}000$ is 134,100, about $67\%$ of the budget. At $b = 500{,}000$ NSGA-II was not re-instrumented, so RQ4 there is reported as a five-way descriptive comparison only.

**Mechanism: depth versus breadth.** The RQ2 crossover and the RQ4 breadth tax are two views of one structural trade-off between search depth and search breadth under a fixed evaluation budget. On this rugged combinatorial landscape (the $37! \approx 1.37 \times 10^{43}$ permutation state space of §3.1), escaping local optima rewards sustained trajectory depth. A scalar Markov chain such as SA spends its entire budget extending a single trajectory, maximizing depth; NSGA-II spreads the same budget across a population front, truncating the depth of any one lineage. Under a severely restricted budget NSGA-II is therefore at a structural disadvantage, which is why at $b = 1{,}000$ it does not exceed SA on hypervolume. As the budget grows its breadth matures and the comparison crosses over (significant from $b = 5{,}000$), yet the effect stays negligible at every budget: even at $b = 500{,}000$, where the population dynamics are fully realized, SA remains within a negligible effect of the dedicated multi-objective optimizer. That a single scalar chain holds parity with NSGA-II across the entire budget range is the stronger-than-expected result.
