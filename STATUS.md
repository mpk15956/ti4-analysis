...
**Last updated:** 2026-06-14 (close-out arc, step A: RQ4 instrumentation landed on elis)
...
| Phase 6/7 finalize (Track B + RQ2/RQ3) | RQ2/RQ3 numbers | `output/paper1_500k_finalize_phase67_20260606_101032/` | Complete; `manuscript_values_phase6.json` present |
...
| C | Doc-drift sweep (5:5:3 comment, manifest header, this file, "advantage" language) | elis | **Done this session** |
| D | Review + push (6 commits + A/C) | user | pending; does NOT block §3.10 or the re-run |
| E | `.sif` rebuild + provenance record + in-SIF canary | GACRC (sbatch) | pending push |
| F | Multi-algo RE-RUN with the A fix = single canonical source for RQ2/RQ3/RQ4; finalize Phase 6/7; regenerate json | GACRC (sbatch) | pending E |
| G | Fold RQ2/RQ4 numbers into §3.10; add RQ4 manifest entries (step B); full gate | GACRC | pending F + prose |
...
1:1:1 composite-best eval index via a return-ignored callback (proven pure).
...
| §3.10 RQ4 | result line + omnibus numbers | step F re-run (`rq4_friedman_evals_to_best.csv`) |
...
| Suite | State |
|-------|-------|
| `tests/test_nsga2_optimizer.py` | 22 passed, incl. new `test_trajectory_callback_is_pure_sink` (RQ4 hook is a byte-identical pure sink) |
| RQ4 generator regimes (`generate_manuscript_values.rq4_evals_to_best`) | absent→unavailable, refused/missing-algo→raise, valid→dict (4 regimes checked) |
| Friedman sentinel guard (`analyze_benchmark.friedman_test`) | REFUSED on a fully-sentinel column; clean chi2/df=5/all-six on real data |

Not yet re-run here: the full suite and `test_phase6_canonical_values.py` (its
fixture skips without the finalize dir; the new RQ4 pins are **red until step F**
by design). The full pre-submission gate runs on GACRC where the canonical
artifacts live.
